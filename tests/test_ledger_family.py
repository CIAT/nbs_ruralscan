...
def test_facts_for_aggregates_or_selects_family():
    facts = {
        ("agroforestry", "T4", "grey", "agroforestry__planted_silvoarable"): {
            "ev": 11,
            "reviewed": 0,
        },
        ("agroforestry", "T4", "grey", "agroforestry__silvopastoral"): {
            "ev": 3,
            "reviewed": 1,
        },
    }
    # specific family
    assert (
        L.facts_for(facts, "agroforestry", "T4", "grey", "agroforestry__silvopastoral")[
            "ev"
        ]
        == 3
    )
    # family='' sums over families
    assert L.facts_for(facts, "agroforestry", "T4", "grey", "")["ev"] == 14
    # a family never searched/extracted → zero (NOT inferred from a sibling family)
    assert (
        L.facts_for(
            facts, "agroforestry", "T4", "grey", "agroforestry__regeneration_farmland"
        )["ev"]
        == 0
    )
...
def test_mark_family_roundtrip(tmp_path, monkeypatch):
    ...
    L.mark("agroforestry", "T4", "grey", "searched", "done")  # family='' (table-level)
    L.mark(
        "agroforestry",
        "T4",
        "grey",
        ...
        note="no FMNR-targeted T4 grey search yet",
    )
    rows = L._rows()
    assert len(rows) == 2
    tbl = {
    ...
    assert tbl[("agroforestry", "T4", "grey", "")]["searched"] == "done"
    fam = tbl[("agroforestry", "T4", "grey", "agroforestry__regeneration_farmland")]
