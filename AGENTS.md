...
- **QA/QC review system (locked).** Human review of flagged evidence happens in the dashboard QA/QC tab + a local `schema_tools/review_server.py` (`:8765`, run `uv run python3 -m nbs_ruralscan.schema_tools.review_server`). Modes: **AI-flagged** (open queue, nobody reviewed) · **AI-passed sample** (spot-check false negatives) · **2nd opinion** (resolved by one reviewer, needs another, per-reviewer) · **My reviewed** (applied history) · **Stats** (AI confusion matrix from `review_log.csv`). Decisions key `(evidence_id × reviewer)` in `pipeline/review/decisions.json`; **Apply** = consensus (agree→write register + `review_log`; disagree→conflict pending). Applied decisions are **viewable, re-openable, and challengeable** (a 2nd reviewer's disagreement auto-reopens a conflict). Reason codes incl. `wrong_practice` (mapped to wrong NbS) and `species_envelope` (per-species row ≠ practice rule). Tabular quotes get an on-demand **table screengrab** (`/api/crop`, page-region render from the cached PDF). Full map: memory `project_qaqc_review_system.md`. **Apply writes LOCAL files only (EV register + `review_log` + regenerated JSON); it does NOT git-commit or push.** To get a review batch into `main`, run **`bash scripts/submit-review.sh <handle> ["title"]`**: it branches fresh off latest `main`, replays your `decisions.json` onto the *current* registers via `schema_tools/submit_review.py` (consensus reduction; surgical → never reverts others' rows), regenerates, commits the registers + JSON, pushes, and opens a PR. This is the supported path — reviewing on a stale branch + merging it wholesale reverts unrelated work and floods the diff with generated-JSON churn. **Run it once per review session** (it ships every agreed decision in one PR — not per flag). Add **`--auto`** to squash-merge the PR immediately (main is unprotected, content is low-risk review metadata) so a batch needs zero clicks; omit it to leave the PR for a second look.
