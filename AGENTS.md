...
- **Enabling-environment variables — ratified routing (2026-06-22).** `distance_to_road` · `accessibility_travel_time`/`market_access` · `market_value_chain` · `tenure_security` · `extension_governance` · `finance_credit_access` · `labour_availability` (+ FMNR `rootstock_presence`, governance) live in **T4 with `suitability_dimension = operational_constraint` + `is_scenario_candidate = true`** — **soft scenario levers** that **also feed the M2b Stream-B operational-risk filter**. They are **NOT** core biophysical/system suitability (don't let them set the base opportunity surface) and **NOT T3** (T3 = NbS × hazard × farming-system). The recurring reviewer question *"is this T4 or T3?"* → **T4 operational-constraint scenario lever (also M2b)**. The agroforestry T4 recipe already classifies all of these correctly; their EV rows keep `use_role = structural_suitability` (→ T4). Don't promote them to a biophysical/system row; a hard-exclusion source is the only `is_scenario_candidate = false` case.
