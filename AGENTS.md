...
- **Bounded, authority-weighted discovery seed-set per NbS** (T4 method §3): WB rural-NbS catalogue · GEF / NBS Invest · IPCC · FAO · WRI · major meta-analyses · MEL/MELIA reports · CSA adoption & barriers dataset. Don't sweep 100k abstracts. Tie selection to `SRC.benchmark_tier`. **Corpus differs per table** — T6/T3 discovery is **deferred** (2026-09 scope cut, see the T3/T6 deferral lock below). Read each paper once: extract the live-role variables (T4 structural + M2b operational_risk) in a single pass.
...
- **T3/T6 extraction DEFERRED (locked, 2026-09-17, ruleset v1.5.0).** The extraction exercise was over-scoped — T3/T6 extracts were nearly all unusable, got weaker automated QA than T4, and the synthesis engine never read them. **Scope cut, not deletion:** active `TABLES` are **T4-only** in `schema_tools/ledger.py` + `search_log.py` + the SRCH manifest enum; **do NOT emit `use_role = climate_risk | nbs_effect | priority_need`** (live roles: `structural_suitability` · `operational_risk` · `dataset`); the dashboard is T4-only. Archived under `schema/registers/_deferred/` (tracked — NOT the gitignored `_quarantine/`): 264 EV rows (88 climate_risk · 176 nbs_effect, incl. the riparian pilot's 2), 157 ledger rows, 155 SRCH rows; T3/T6 extraction contracts under `.agents/skills/_versions/v1.4.2/contracts/`; `_T3`/`_T6` discovery logs stay on disk but leave the dashboard payload. **T3/T6 schema specs + recipe CSVs are KEPT and FROZEN** (M2b Stream A reads T3 fields; M5/T5 join on T6 later) — don't populate them. Two SRC rows keep `vars_extracted=""` (`quandt_resilience_2017`, `grass_barrier_vfs_runoff_effectiveness_2004`). Restore procedure: `schema/registers/_deferred/README.md`.
...
- `docs/` — GitHub Pages site (wireframe, pipeline diagram, index, schema page + ERD, **`dashboard.html`** — Evidence & Literature Interrogation Dashboard: merged Evidence & Discovery tab w/ acquisition-funnel coverage matrix + QA/QC review UI; reads `dashboard_data.json` + `progress.json`, both generated)
