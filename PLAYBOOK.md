...
1. Start the local review server: `uv run python3 -m nbs_ruralscan.schema_tools.review_server` → http://localhost:8765/dashboard.html → **QA/QC** tab. (One-time per clone: `bash scripts/setup-repo.sh`; be logged in — `gh auth status`; **and hydrate the corpus — `NBS_LIBRARY_ROOT="<your OneDrive>/.../1_Projects" python3 scripts/hydrate-corpus.py`.** Apply's guardrail verifies every quote against the local, gitignored `.cache/corpus/`, so a fresh clone that skips this fails with **GUARDRAIL FAILED** listing missing PDFs — issue #181. Set `NBS_LIBRARY_ROOT` if your OneDrive folder name differs, e.g. on Windows.)
