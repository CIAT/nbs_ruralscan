...
- **Enabling-environment = T4 operational-constraint scenario levers (ratified 2026-06-22).** `market_access`/`accessibility_travel_time` · `market_value_chain` · `tenure_security` · `extension_governance` · `finance_credit_access` · `labour_availability` · `distance_to_road` go in **T4 with `suitability_dimension=operational_constraint` + `is_scenario_candidate=true`** — soft scenario levers that **also feed M2b Stream-B**. So `use_role=structural_suitability` is correct (→T4), but they are **NOT core biophysical/system suitability** (don't let them set the base opportunity surface) and **NOT T3** (T3 = hazard × farming-system). Answer to "T4 or T3?" = T4 operational scenario lever (also M2b). A hard-exclusion source is the only `is_scenario_candidate=false` case.
